...
export default function SupportRealtimeNotifier({ initialLatestTicketId = "", initialOpenCount = 0 }) {
  const [permission, setPermission] = useState("default");
  const [openCount, setOpenCount] = useState(Number(initialOpenCount || 0));
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const nextOpenCount = Number(data?.stats?.openCount || 0);
        ...
        if (hasNewTicket && !recentlyStarted) {
          setLastMessage("وصل طلب دعم جديد");
          playBeep();

          if (canUseBrowserNotification() && Notification.permission === "granted") {
            new Notification("Dribdo Support", {
              body: "وصل طلب دعم جديد في صندوق الرسائل.",
            });
          }
        }

        if (nextLatestId) {
          latestTicketRef.current = nextLatestId;
        }
      } catch {
        // Ignore transient network issues for polling.
      } finally {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
      ...
      <div className="mt-1">الطلبات المفتوحة الآن: {openCount}</div>
