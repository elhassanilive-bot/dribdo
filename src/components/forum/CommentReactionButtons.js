...
export default function CommentReactionButtons({ commentId }) {
  ...
  function toggle(reaction) {
    ...
    startTransition(async () => {
      ...
      if (existing && existing.reaction === reaction) {
        const { error } = await supabase.from("blog_comment_reactions").delete().eq("id", existing.id);
        if (!error) {
          setMyReaction("");
      ...
      if (existing) {
        ...
        if (!error) {
          ...
          setCounts((c) => ({
            like: c.like + (reaction === "like" ? 1 : -1),
            dislike: c.dislike + (reaction === "dislike" ? 1 : -1),
          }));
        }
        return;
      }

      const { error } = await supabase.from("blog_comment_reactions").insert({
